function searchDataSQ2023() {
  ...
  // 現在のシート (Sデータ(QUICK)) の最後の行を取得
  var lastRowB = sheetB.getLastRow(); // シートのデータがある最後の行番号
  var startRowB1 = lastRowB + 1;      // S（日経＋QUICK合計）のデータ開始行
  ...
  for (var r = 0; r < matchingRows1.length; r++) {
    ...
    for (var h = 0; h < headersB.length; h++) {
      if (h < 8) { // H列まで
        // Sデータ(QUICK)の5行目のH列までとS（日経＋QUICK合計）3行目の項目名を比較
        ...
        // Sデータ(QUICK)の5行目のH列以降とS（日経＋QUICK合計）2行目の項目名を比較
  ...
  for (var r = 0; r < matchingRows2.length; r++) {
    ...
    for (var h = 0; h < headersB.length; h++) {
      if (h < 8) { // H列まで
        // 確認2023(QuickE)の5行目のJ列までと入力シート（合計QUICKS）3行目の項目名を比較
        ...
        // 確認2023(QuickE)の5行目のJ列以降と入力シート（合計QUICKS）2行目の項目名を比較
