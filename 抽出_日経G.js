function searchDataG2023() {
  ...
  if (!sheetB) {
    Logger.log('Gデータ(日経)が見つかりません: ' + sheetBName);
    SpreadsheetApp.getUi().alert('Gデータ(日経)が見つかりません: ' + sheetBName);
    return;
  }

  // 確認2023(日経G)のC4からH4までのセルの値を取得
  ...
  codesToSearch = codesToSearch.filter(function(code) {
    ...
    // 現在のシート (Gデータ(日経)) に既に存在する銘柄コードを取得
...
for (var r = 0; r < matchingRows.length; r++) {
  ...
  for (var h = 0; h < headersB.length; h++) {
    if (h < 8) { // H列まで
      // 確認2023(日経G)の5行目のH列までとG（日経＋QUICK合計）3行目の項目名を比較
      ...
      // 確認2023(日経G)の5行目のH列以降とG（日経＋QUICK合計）2行目の項目名を比較, headerIndexA2の空白の時の判定処理どうするか？getLastColumnを変えるか。
