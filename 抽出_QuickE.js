function searchDataQE2023() {
  ...
  if (!sheetB) {
    Logger.log('Eデータ(QUICK)が見つかりません: ' + sheetBName);
    SpreadsheetApp.getUi().alert('Eデータ(QUICK)が見つかりません: ' + sheetBName);
    return;
  }

  // Eデータ(QUICK)のC4からH4までのセルの値を取得
  ...
  codesToSearch = codesToSearch.filter(function(code) {
    ...
    // 現在のシート (Eデータ(QUICK)) に既に存在する銘柄コードを取得
  ...
  for (var r = 0; r < matchingRows2.length; r++) {
    ...
    for (var h = 0; h < headersB.length; h++) {
      if (h < 8) { // H列まで
        // 確認2023(QuickE)の5行目のJ列までと入力シート（合計QUICKE）3行目の項目名を比較
        ...
        // 確認2023(QuickE)の5行目のJ列以降と入力シート（合計QUICKE）2行目の項目名を比較
