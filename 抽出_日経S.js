function searchDataS2023() {
  ...
  if (!sheetB) {
    Logger.log('Sデータ(日経)が見つかりません: ' + sheetBName);
    SpreadsheetApp.getUi().alert('Sデータ(日経)が見つかりません: ' + sheetBName);
    return;
  }

  // 確認2023(日経S)のC4からH4までのセルの値を取得
  ...
  codesToSearch = codesToSearch.filter(function(code) {
    ...
    // 現在のシート (Sデータ(日経)) に既に存在する銘柄コードを取得
...
for (var r = 0; r < matchingRows.length; r++) {
  ...
  for (var h = 0; h < headersB.length; h++) {
    if (h < 8) { // H列まで
      // 確認2023(日経S)の5行目のH列までとS（日経＋QUICK合計）3行目の項目名を比較
      ...
      // 確認2023(日経S)の5行目のH列以降とS（日経＋QUICK合計）2行目の項目名を比較, headerIndexA2の空白の時の判定処理どうするか？getLastColumnを変えるか。
